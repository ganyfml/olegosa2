...
bool Seq::load_SA()
{
	seqan::Index<SeqString, seqan::IndexEsa<> > search_index;
	if(!open(search_index, toCString(index_file_)))
	{
	...
	else
	{
		...
		search_index_ = make_shared<seqan::Index<SeqString, seqan::IndexEsa<> >>(search_index); 
	}
