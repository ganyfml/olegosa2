...
{
	return seqan::indexCreate(*const_cast<T*>(static_cast<const T*>(seqIndex.get_pointer())), seqan::FibreSA());
}

bool openIndex(SeqIndex& seqIndex, const std::string& fileName)
{
	return seqan::open(*const_cast<T*>(static_cast<const T*>(seqIndex.get_pointer())), seqan::toCString(fileName));
}	

bool saveIndex(SeqIndex& seqIndex, const std::string& fileName)
{
	return seqan::save(*const_cast<T*>(static_cast<const T*>(seqIndex.get_pointer())), seqan::toCString(fileName));
}	
